...
async fn test_assemble_context_with_search_results() {
    ...
    // Mock get_db() for pinned messages lookup
    mock_repo
        .expect_get_db()
        .returning(|| panic!("DB access not needed in mock"));
    ...
    // Context assembly will try to access DB for pinned messages
    // Without a real DB, this will fail, which is expected in unit tests
...
async fn test_assemble_context_with_excluded_folders() {
    ...
    // Mock get_db() for pinned messages
    mock_repo
        .expect_get_db()
        .returning(|| panic!("DB access not needed in mock"));
...
async fn test_score_message_importance() {
    ...
    mock_repo.expect_find_message_by_id().returning(move |_| {
        Ok(Some(Message {
            id: test_message_id,
...
async fn test_suggest_pruning() {
    ...
    // Mock get_db() for additional queries
    mock_repo
        .expect_get_db()
        .returning(|| panic!("DB access not needed in mock"));
...
async fn test_assemble_context_prefers_labels() {
    ...
    // Mock get_db() for pinned and recent messages
    mock_repo
        .expect_get_db()
        .returning(|| panic!("DB access not needed in mock"));
