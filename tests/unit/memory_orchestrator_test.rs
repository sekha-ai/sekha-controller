use sekha_controller::{
    ...
    orchestrator::{MemoryOrchestrator, pruning_engine::PruningSuggestion, label_intelligence::LabelSuggestion},
...
async fn test_suggest_pruning() {
    ...
    mock_repo
        ...
        .returning(move |_, _, _| Ok((old_convs.clone(), old_convs.len())));
...
async fn test_assemble_context_respects_budget() {
    ...
    let total_content_length: usize = messages.iter()
        .map(|m| m.content.len())
        .sum();
