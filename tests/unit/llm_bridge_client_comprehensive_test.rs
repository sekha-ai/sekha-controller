...
async fn test_embed_text_success() {
    ...
    let _mock_route = server.mock("POST", "/v2/route")
        .with_status(200)
    ...
    let _mock_embed = server.mock("POST", "/v2/embeddings")
        .with_status(200)
        .with_header("content-type", "application/json")
...
async fn test_summarize_with_custom_max_words() {
    ...
    let result = client.summarize(messages, "weekly", Some("gpt-4"), Some(50)).await;
