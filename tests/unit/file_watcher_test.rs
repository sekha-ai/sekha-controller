...
## Assistant
Markdown unit test response"#.to_string()
}

fn txt_content() -> String {
    r#"User: TXT unit test message
Assistant: TXT unit test response"#.to_string()
}
...
async fn test_process_chatgpt_single_format() {
    ...
    assert!(result.is_ok(), "Should process ChatGPT single format: {:?}", result);
    
    let conversations = repo.find_by_label("ChatGPT Single Unit Test", 10, 0)
        .await
        .unwrap();
...
async fn test_process_claude_format() {
    ...
    let conversations = repo.find_by_label("Claude JSON Unit Test", 10, 0)
        .await
        .unwrap();
    assert_eq!(conversations.len(), 1);
}
...
async fn test_process_markdown_format() {
    ...
    assert!(result.is_ok(), "Should process markdown format: {:?}", result);
...
async fn test_process_empty_messages() {
    ...
    let conversations = repo.find_by_label("Empty Messages Test", 10, 0)
        .await
        .unwrap();
    assert_eq!(conversations.len(), 1);
...
async fn test_process_long_content() {
    ...
    let conversations = repo.find_by_label("Long Content Test", 10, 0)
        .await
        .unwrap();
...
async fn test_process_special_characters() {
    ...
    let conversations = repo.find_by_label("Special Chars Test !@#$%^&*()", 10, 0)
        .await
        .unwrap();
    assert_eq!(conversations.len(), 1);
    assert!(conversations[0].word_count > 0, "Should have messages imported (word_count > 0)");
}
