...
class SekhaClient:
    ...
    @backoff.on_exception(backoff.expo, (SekhaConnectionError, httpx.TimeoutException), max_tries=3)
    ...
    async def get_conversation(self, conversation_id: str) -> ConversationResponse:
        ...
        try:
            ...
            raise SekhaAPIError("Failed to get conversation", e.response.status_code, e.response.text)
    ...
    async def delete_conversation(self, conversation_id: str) -> None:
        ...
        try:
            ...
            if e.response.status_code == 404:
                raise SekhaNotFoundError(f"Conversation {conversation_id} not found")
            raise SekhaAPIError("Failed to delete conversation", e.response.status_code, e.response.text)
