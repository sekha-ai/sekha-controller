...
use sea_orm::{QueryFilter, ColumnTrait}; // REMOVE EntityTrait from here
...
impl HierarchicalSummarizer {
    ...
    pub async fn generate_daily_summary(
        ...
        let messages = self.fetch_messages_from_last_n_days(conversation_id, 1).await?;
        
        if messages.is_empty() {
            return Ok("No messages to summarize".to_string());
        }
        
        let messages_text: Vec<String> = messages.iter()
            .map(|m| format!("[{}] {}: {}", m.timestamp, m.role, m.content))
            .collect();
        
        let summary = self.llm_bridge.summarize(
            messages_text,
            "daily",
            None,
            Some(200),
        ...
        self.store_summary(conversation_id, "daily", &summary).await?;
    ...
    pub async fn generate_weekly_summary(
        ...
        let daily_summaries = self.fetch_summaries_from_last_n_days(conversation_id, 7, "daily").await?;
        ...
        self.store_summary(conversation_id, "weekly", &summary).await?;
    ...
    pub async fn generate_monthly_summary(
        ...
        let weekly_summaries = self.fetch_summaries_from_last_n_days(conversation_id, 30, "weekly").await?;
        ...
        self.store_summary(conversation_id, "monthly", &summary).await?;
    ...
    async fn fetch_messages_from_last_n_days(
        ...
        Ok(models.into_iter().map(|m| Message {
            ...
            embedding_id: m.embedding_id.as_ref().and_then(|id| Uuid::parse_str(id).ok()),
            metadata: m.metadata.as_ref().and_then(|meta| serde_json::from_str(meta).ok()),
            }).collect())
    }
    ...
    async fn store_summary(
        ...
        tracing::info!("Stored {} summary for {}: {}", level, conversation_id, summary.chars().take(50).collect::<String>());
        Ok(())
    }
}